...
/* ----- Global Variables ---------------------------------------- */
	...
	function Game() {
		...
		this.pauseResume = function () {
			if (!this.running) {
				...
				this.showMessage("Pauzė","Spausk belenkur");
				}
			};
	...
	function Ghost(name, gridPosX, gridPosY, gridBaseX, gridBaseY) {
		...
		this.draw = function (context) {
			if (this.dead) {
				...
				var idx = animationIndex + animationDirection;
				if (idx >= pngs.length || idx < 0) {
					animationDirection = animationDirection * -1;
					idx = animationIndex + animationDirection;
				}
				animationIndex = idx;
				context.drawImage(pngs[animationIndex], this.posX, this.posY, 2*this.radius, 2*this.radius);
			}
		}
	...
	function pacman() {
		...
		for (var i = 1; i < 10; i++) {
			var img = new Image();
			img.src = 'img/valdas/valdas' + i + '.png';
		...
		this.draw = function (context) {
			...
			animationIndex = idx;
			context.drawImage(this.images[animationIndex], this.posX, this.posY, 2*this.radius, 2*this.radius);
		}
